        ...
        <div className="ria-create-grid" style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: "1.5rem", alignItems: "start" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div style={sectionStyle}>
              <label htmlFor="create-title" style={labelStyle}>Titel des Inserats <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
              <input id="create-title" name="title" value={formData.title} onChange={handleChange} placeholder="z. B. Campingzelt, Kamera, E-Roller ..." aria-required="true" onFocus={applyInputFocus} onBlur={resetInputFocus} style={{ ...inputBaseStyle, fontSize: "1.05rem", fontWeight: 600 }} />
            </div>

            {/* Category chips */}
            <div style={sectionStyle}>
              <label style={labelStyle} id="category-label">Kategorie <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
                ...
                {allCategoryNames.map((cat) => {
                  ...
                  return (
                    <button key={cat} type="button" onClick={() => setCategory(cat)} aria-pressed={active} style={{ padding: "0.5rem 0.85rem", borderRadius: 999, border: active ? `2px solid ${C.forest}` : `1px solid ${C.line}`, background: active ? C.forest : "white", color: active ? "white" : C.ink, fontWeight: active ? 700 : 500, fontSize: "0.88rem", cursor: "pointer", transition: "all 0.18s ease", display: "flex", alignItems: "center", gap: "0.35rem" }}>
                      <span aria-hidden="true">{categoryIcons[cat]}</span>{cat}
                    </button>
                  );
                })}
              </div>
            ...
            <div style={sectionStyle}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
                <div>
                  <label htmlFor="create-price" style={labelStyle}>Preis <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
                  <input id="create-price" name="price" value={formData.price} onChange={handleChange} placeholder="z. B. 8€ / Tag" aria-required="true" onFocus={applyInputFocus} onBlur={resetInputFocus} style={{ ...inputBaseStyle, fontWeight: 700 }} />
                ...
                <div>
                  <label htmlFor="create-location" style={labelStyle}>Ort <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
                  <input id="create-location" name="location" value={formData.location} onChange={handleChange} placeholder="Deine Stadt oder Stadtteil" aria-required="true" onFocus={applyInputFocus} onBlur={resetInputFocus} style={inputBaseStyle} />
                </div>
              </div>
            </div>
            ...
            {/* Description */}
            <div style={sectionStyle}>
              <label htmlFor="create-description" style={labelStyle}>Beschreibung <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
              <textarea id="create-description" name="description" value={formData.description} onChange={handleChange} placeholder="Kurz den Zustand beschreiben, was dabei ist und ob Abholung oder Übergabe vor Ort möglich ist..." rows={5} aria-required="true" onFocus={applyInputFocus} onBlur={resetInputFocus} style={{ ...inputBaseStyle, resize: "vertical", lineHeight: 1.65 }} />
            ...
            {/* Extra images */}
            <div style={sectionStyle}>
              <label style={labelStyle}>Weitere Bilder <span style={{ color: C.muted, fontWeight: 400 }}>(optional)</span></label>
              <label style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", padding: "0.65rem 1rem", borderRadius: 12, background: C.sageLight, color: C.forest, fontWeight: 700, cursor: "pointer", fontSize: "0.85rem", border: `1px solid ${C.line}`, marginBottom: "0.75rem" }}>
