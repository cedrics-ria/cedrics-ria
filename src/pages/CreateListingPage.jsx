...
const STEPS = [
  { num: 1, label: 'Grundlagen', desc: 'Titel & Kategorie' },
  { num: 2, label: 'Details', desc: 'Preis, Ort & Beschreibung' },
  { num: 3, label: 'Extras', desc: 'Bilder & Optionales' },
];
...
const visibleCategories = allCategoryNames.filter((c) => c !== 'Outdoor & Sport');
...
export default function CreateListingPage({ onAddListing, goTo, currentUser, addToast }) {
  ...
  const previewImage =
    !imgError && formData.image ? formData.image : getFallbackImage(formData.category || 'Sonstiges');

  // Step validation
  const step1Valid = formData.title.trim() && formData.category;
  const step2Valid = formData.price.trim() && formData.location.trim() && formData.description.trim();

  function nextStep() {
    if (step === 1 && !step1Valid) { addToast('Bitte Titel und Kategorie ausfüllen.', 'error'); return; }
    if (step === 2 && !step2Valid) { addToast('Bitte Preis, Ort und Beschreibung ausfüllen.', 'error'); return; }
    setStep((s) => s + 1);
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.cream, padding: '3rem 1.5rem 5rem' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        {/* Back */}
        <button
          onClick={() => step > 1 ? setStep((s) => s - 1) : goTo('listings')}
          style={{ background: 'white', color: C.forest, padding: '0.6rem 1rem', borderRadius: 12, border: `1px solid ${C.line}`, fontWeight: 600, cursor: 'pointer', marginBottom: '2rem', fontSize: '0.9rem' }}
        >
        ...
        <div style={{ marginBottom: '2rem' }}>
          <p style={{ fontSize: '0.75rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: C.sage, fontWeight: 700, marginBottom: '0.4rem' }}>
            Neues Inserat
          </p>
          <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', color: C.forest, margin: 0, letterSpacing: '-0.03em' }}>
            {step === 1 ? 'Was möchtest du verleihen?' : step === 2 ? 'Preis, Ort & Beschreibung' : 'Fast fertig!'}
          </h1>
        </div>
              ...
              <div key={s.num} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: i < STEPS.length - 1 ? 1 : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                  <div style={{
                    ...
                    background: done ? C.sage : active ? C.forest : 'white',
                    border: `2px solid ${done ? C.sage : active ? C.forest : C.line}`,
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: active ? C.forest : done ? C.sage : C.muted, lineHeight: 1.1 }}>{s.label}</span>
                    <span style={{ fontSize: '0.7rem', color: C.muted, lineHeight: 1.1 }}>{s.desc}</span>
                  </div>
                </div>
                {i < STEPS.length - 1 && (
                  <div style={{ flex: 1, height: 2, borderRadius: 99, background: done ? C.sage : C.line, marginLeft: '0.5rem', transition: 'background 0.3s ease' }} />
                )}
              </div>
            );
          })}
        ...
        <div className="ria-create-grid" style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '1.5rem', alignItems: 'start' }}>
              ...
              <>
                <div style={sectionStyle}>
                  <label htmlFor="create-title" style={labelStyle}>
                    Titel des Inserats <span style={{ color: C.terra }}>*</span>
                  </label>
                  <input
                    id="create-title"
                    name="title"
                ...
                <div style={sectionStyle}>
                  ...
                  <div role="group" aria-labelledby="category-label" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                ...
                <button
                  ...
                  onClick={nextStep}
                  style={{
                    background: step1Valid ? 'linear-gradient(135deg, #163126, #1C3A2E)' : C.muted,
                    color: 'white', padding: '1.1rem', borderRadius: 16, border: 'none',
                    fontSize: '1rem', fontWeight: 800, cursor: step1Valid ? 'pointer' : 'not-allowed',
                    boxShadow: step1Valid ? '0 14px 34px rgba(28,58,46,0.28)' : 'none',
              ...
              <>
                <div style={sectionStyle}>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.6rem', flexWrap: 'wrap' }}>
                    {['3€ / Tag', '5€ / Tag', '8€ / Tag', '15€ / Tag'].map((p) => (
                      <button key={p} type="button" onClick={() => setFormData((prev) => ({ ...prev, price: p }))}
                        style={{ padding: '0.25rem 0.65rem', borderRadius: 999, border: `1px solid ${C.line}`, background: formData.price === p ? C.terra : 'white', color: formData.price === p ? 'white' : C.muted, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }}>
                        {p}
                      </button>
                    ))}
                  </div>
                ...
                <div style={sectionStyle}>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem' }}>
                    ...
                    <div>
                      <label htmlFor="create-plz" style={labelStyle}>PLZ</label>
                      <input
                        id="create-plz"
                        name="plz"
                        value={formData.plz}
                ...
                <div style={sectionStyle}>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    placeholder="Kurz den Zustand beschreiben, was dabei ist, ob Abholung oder Übergabe vor Ort möglich ist..."
                    rows={5}
                    maxLength={1000}
                    onFocus={applyInputFocus}
                    onBlur={resetInputFocus}
                  ...
                  />
                  <div style={{ textAlign: 'right', fontSize: '0.78rem', color: formData.description.length > 800 ? C.terra : C.muted, marginTop: '0.4rem' }}>
                ...
                <button
                  ...
                  onClick={nextStep}
                  style={{
                    background: step2Valid ? 'linear-gradient(135deg, #163126, #1C3A2E)' : C.muted,
                    color: 'white', padding: '1.1rem', borderRadius: 16, border: 'none',
                    fontSize: '1rem', fontWeight: 800, cursor: step2Valid ? 'pointer' : 'not-allowed',
                    boxShadow: step2Valid ? '0 14px 34px rgba(28,58,46,0.28)' : 'none',
                    transition: 'all 0.3s ease',
                  }}
                >
                  Weiter → Bilder & Extras
                </button>
              </>
            )}

            {/* ── STEP 3: Bilder + Kaution + Zahlung ── */}
            {step === 3 && (
              <>
                {/* Main image */}
                <div style={sectionStyle}>
                  <label style={labelStyle}>
                    Titelbild <span style={{ color: C.muted, fontWeight: 400 }}>(optional)</span>
                  </label>
                  ...
                  <p style={{ margin: '0.6rem 0 0', fontSize: '0.8rem', color: C.muted }}>Kein Bild? Wir nutzen automatisch ein Kategoriebild.</p>
                </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      {extraImages.map((url, i) => (
                        <div key={i} style={{ position: 'relative' }}>
                          <img src={url} alt="" style={{ width: 72, height: 58, objectFit: 'cover', borderRadius: 10, border: `1px solid ${C.line}` }} />
                          <button type="button" onClick={() => setExtraImages((prev) => prev.filter((_, j) => j !== i))} style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%', background: C.terra, color: 'white', border: 'none', cursor: 'pointer', fontSize: '0.7rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800 }}>×</button>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ marginBottom: '1.25rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.6rem', flexWrap: 'wrap' }}>
                      {['Keine', '20 €', '50 €', '100 €'].map((k) => (
                        <button key={k} type="button" onClick={() => setFormData((p) => ({ ...p, kaution: k === 'Keine' ? '' : k }))}
                          style={{ padding: '0.25rem 0.65rem', borderRadius: 999, border: `1px solid ${C.line}`, background: (k === 'Keine' ? !formData.kaution : formData.kaution === k) ? C.terra : 'white', color: (k === 'Keine' ? !formData.kaution : formData.kaution === k) ? 'white' : C.muted, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }}>
                          {k}
                        </button>
                      ))}
                    </div>
                      ...
                      {allPaymentMethods.map(({ id, label }) => {
                        ...
                        return (
                          <button key={id} type="button" onClick={() => togglePayment(id)}
                            style={{ padding: '0.4rem 0.85rem', borderRadius: 999, border: active ? `2px solid ${C.forest}` : `1px solid ${C.line}`, background: active ? C.forest : 'white', color: active ? 'white' : C.ink, fontWeight: active ? 700 : 500, fontSize: '0.85rem', cursor: 'pointer' }}>
                            {label}
                          </button>
                        );
                      })}
                ...
                <button
                  ...
                  style={{
                    ...
                    color: 'white', padding: '1.1rem 1.5rem', borderRadius: 16, border: 'none',
                ...
                >
                  {submitting ? 'Wird veröffentlicht…' : '🎉 Inserat veröffentlichen'}
                </button>
              </>
            )}
          </form>
          ...
          <div style={{ position: 'sticky', top: '6rem' }}>
            <p style={{ fontSize: '0.75rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: C.sage, fontWeight: 700, marginBottom: '0.75rem' }}>
              Vorschau
            </p>
              ...
              <div style={{ padding: '1.4rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '0.75rem', marginBottom: '0.5rem' }}>
                  <h3 style={{ color: C.forest, margin: 0, fontSize: '1.15rem', lineHeight: 1.3 }}>
                    {formData.title || <span style={{ color: C.muted, fontStyle: 'italic', fontWeight: 400 }}>Dein Titel...</span>}
                  </h3>
                  <span style={{ color: C.terra, fontWeight: 800, whiteSpace: 'nowrap', fontSize: '1rem' }}>
                    {formData.price || <span style={{ color: C.muted, fontStyle: 'italic', fontWeight: 400 }}>Preis</span>}
                  </span>
                </div>
                ...
                <p style={{ color: C.ink, fontSize: '0.9rem', lineHeight: 1.65, margin: '0 0 1rem', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                  {formData.description || <span style={{ color: C.muted, fontStyle: 'italic' }}>Deine Beschreibung...</span>}
                </p>
            ...
            <div style={{ marginTop: '1.25rem', background: 'white', borderRadius: 18, padding: '1.25rem', border: `1px solid ${C.line}` }}>
              <p style={{ margin: '0 0 0.75rem', fontSize: '0.8rem', fontWeight: 700, color: C.forest }}>Fortschritt</p>
              {[
                ['Titel', formData.title, 1],
                ['Kategorie', formData.category, 1],
                ['Preis', formData.price, 2],
                ['Ort', formData.location, 2],
                ['Beschreibung', formData.description, 2],
                ['Bild', formData.image, 3],
              ].map(([label, val, stepNum]) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.45rem' }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', border: `2px solid ${val ? C.sage : C.line}`, background: val ? C.sage : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s ease' }}>
                    {val && <svg width="8" height="8" viewBox="0 0 10 10"><polyline points="1.5,5.5 4,8 8.5,2" stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                  </div>
                  <span style={{ fontSize: '0.82rem', color: val ? C.forest : C.muted, fontWeight: val ? 600 : 400 }}>
                    {label}
                    {!val && stepNum > step && <span style={{ fontSize: '0.7rem', color: C.muted, fontWeight: 400 }}> · Schritt {stepNum}</span>}
                  </span>
                </div>
              ))}
            </div>
