...
export default function HomePage({ goTo, listings, currentUser, onSeedDemo, onCategoryClick, onSearch }) {
      ...
      <section className="ria-hero-section" style={{ background: "linear-gradient(135deg, #173126 0%, #1C3A2E 48%, #244536 100%)", color: "white", padding: "6rem 1.5rem 5rem", position: "relative", overflow: "hidden" }}>
        ...
        <div className="ria-hero-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "center" }}>
          <div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); const v = e.target.elements.hs.value.trim(); if (v && onSearch) onSearch(v); else if (v) goTo("listings"); }} style={{ marginTop: "1.75rem", display: "flex", background: "rgba(255,255,255,0.10)", backdropFilter: "blur(12px)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.18)", overflow: "hidden", maxWidth: 460, animation: "fadeUp 1s 0.5s both" }}>
              <svg style={{ margin: "0 0 0 1rem", alignSelf: "center", flexShrink: 0, opacity: 0.55 }} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              <input name="hs" placeholder="Bohrmaschine, Zelt, Kamera…" style={{ flex: 1, background: "transparent", border: "none", padding: "0.9rem 0.85rem", color: "white", fontSize: "0.95rem", outline: "none" }} />
              <button type="submit" style={{ background: C.terra, border: "none", padding: "0.9rem 1.25rem", color: "white", cursor: "pointer", fontWeight: 700, fontSize: "0.9rem", flexShrink: 0, transition: "background 0.2s" }}>Suchen</button>
            </form>
          </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: "1rem", flexWrap: "wrap", marginBottom: "1.5rem" }}>
            <div>
              ...
              <h2 style={{ fontSize: "2.1rem", color: C.forest, margin: 0, letterSpacing: "-0.03em" }}>{listings.length > 0 ? `${listings.length} Inserate in deiner Nähe` : "Beliebte Inserate"}</h2>
            </div>
          ...
          {listings.length === 0 && currentUser && onSeedDemo && (
            <div style={{ gridColumn: "1 / -1", background: "white", borderRadius: 20, padding: "2rem", border: `1px dashed ${C.line}`, textAlign: "center", marginBottom: "1rem" }}>
              <div style={{ fontSize: "1.5rem", marginBottom: "0.5rem" }}>🌱</div>
              <div style={{ fontWeight: 700, color: C.forest, marginBottom: "0.4rem" }}>App noch leer</div>
              <p style={{ color: C.muted, fontSize: "0.88rem", margin: "0 0 1rem", lineHeight: 1.6 }}>Füge 8 Demo-Inserate hinzu, damit die App sofort gut aussieht.</p>
              <button onClick={onSeedDemo} style={{ background: C.forest, color: "white", padding: "0.75rem 1.5rem", borderRadius: 12, border: "none", fontWeight: 700, cursor: "pointer", fontSize: "0.9rem" }}>Demo-Inserate hinzufügen</button>
            </div>
          )}
              ...
              <div key={item.id} className="hover-card" style={{ background: "white", borderRadius: 24, overflow: "hidden", border: `1px solid ${C.line}`, boxShadow: C.shadow }}>
                <div style={{ position: "relative", height: 220 }}>
                  ...
                  {item.isAvailable === false
                    ? <div style={{ position: "absolute", top: 12, left: 12, background: C.terra, color: "white", padding: "0.35rem 0.7rem", borderRadius: 999, fontSize: "0.75rem", fontWeight: 800 }}>Vergeben</div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="ria-grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: "1rem" }}>
            {categories.map((category) => (
              <div key={category.name} className="hover-media" onClick={() => onCategoryClick ? onCategoryClick(category.name) : goTo("listings")} style={{ borderRadius: 24, overflow: "hidden", position: "relative", minHeight: 210, animation: "fadeUp 0.6s ease both", boxShadow: C.shadow, border: `1px solid ${C.line}`, cursor: "pointer" }}>
