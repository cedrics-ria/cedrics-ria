...
export default function ListingDetailPage({ listing, goTo, currentUser, onStartMessage, allListings, onSelectListing, favorites, toggleFavorite, addToast, onEditListing, onDeleteListing, onReviewAdded, onToggleAvailability, onViewOwner }) {
  if (!listing) {
    return (
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.cream, padding: "3rem 1.5rem 5rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div style={{ background: "white", borderRadius: 24, padding: "1.75rem", boxShadow: C.shadow, border: `1px solid ${C.line}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.25rem" }}>
                ...
                <button onClick={() => toggleFavorite(listing.id)} style={{ width: 42, height: 42, borderRadius: "50%", background: favorites.includes(String(listing.id)) ? "rgba(196,113,74,0.1)" : C.sageLight, border: `1px solid ${favorites.includes(String(listing.id)) ? C.terra : C.line}`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", color: favorites.includes(String(listing.id)) ? C.terra : C.muted, flexShrink: 0 }}>
              ...
              {listing.ownerName && (
                <div onClick={() => onViewOwner && onViewOwner({ id: listing.userId, name: listing.ownerName })} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "1rem", borderRadius: 14, background: C.sageLight, marginBottom: "1.25rem", cursor: onViewOwner ? "pointer" : "default", transition: "background 0.15s ease" }} onMouseEnter={(e) => { if (onViewOwner) e.currentTarget.style.background = "#dce8dc"; }} onMouseLeave={(e) => { e.currentTarget.style.background = C.sageLight; }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, color: C.forest, fontSize: "0.9rem", display: "flex", alignItems: "center", gap: "0.4rem", flexWrap: "wrap" }}>
                      {listing.ownerName}
                      {listing.ownerVerified && <VerifiedBadge small />}
                    </div>
                    <div style={{ fontSize: "0.78rem", color: C.sage }}>Profil ansehen →</div>
                  </div>
                  {listing.reviews > 0 && <div style={{ color: C.gold, fontWeight: 700, fontSize: "0.9rem" }}>★ {listing.rating.toFixed(1)} <span style={{ color: C.muted, fontWeight: 400, fontSize: "0.78rem" }}>({listing.reviews})</span></div>}
                </div>
              )}

              {currentUser?.id === listing.userId ? (
                <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "0.75rem" }}>
                  <div style={{ background: C.sageLight, borderRadius: 12, padding: "0.75rem 1rem", textAlign: "center", fontSize: "0.85rem", color: C.forest, fontWeight: 600 }}>
                    Das ist dein Inserat
                  </div>
                  <div style={{ display: "flex", gap: "0.6rem" }}>
                    <button onClick={() => onEditListing(listing)} style={{ flex: 1, padding: "0.9rem", borderRadius: 12, border: `1px solid ${C.line}`, background: "white", color: C.forest, fontWeight: 700, cursor: "pointer", fontSize: "0.9rem" }}>
                      Bearbeiten
                    </button>
                    <button onClick={() => onDeleteListing(listing.id)} style={{ flex: 1, padding: "0.9rem", borderRadius: 12, border: "none", background: "rgba(196,113,74,0.1)", color: C.terra, fontWeight: 700, cursor: "pointer", fontSize: "0.9rem" }}>
                      Löschen
                    </button>
                  </div>
                  <button
                    onClick={() => onToggleAvailability(listing.id, !listing.isAvailable)}
                    style={{ width: "100%", padding: "0.85rem", borderRadius: 12, border: `1px solid ${listing.isAvailable ? C.line : C.terra}`, background: listing.isAvailable ? "white" : "rgba(196,113,74,0.07)", color: listing.isAvailable ? C.muted : C.terra, fontWeight: 700, cursor: "pointer", fontSize: "0.88rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}
                  >
                    <span style={{ fontSize: "0.9rem" }}>{listing.isAvailable ? "✓" : "✗"}</span>
                    {listing.isAvailable ? "Verfügbar — als vergeben markieren" : "Vergeben — wieder freigeben"}
                  </button>
                </div>
              ) : listing.isAvailable === false ? (
                <div style={{ background: "rgba(196,113,74,0.08)", border: `1px solid ${C.terra}`, borderRadius: 14, padding: "1.1rem", textAlign: "center", marginBottom: "0.75rem" }}>
                  <div style={{ fontWeight: 800, color: C.terra, marginBottom: "0.2rem" }}>Momentan vergeben</div>
                  <div style={{ fontSize: "0.82rem", color: C.muted }}>Dieses Inserat ist aktuell nicht verfügbar.</div>
                </div>
              ) : (
        ...
        {/* Bewertungen */}
        <ReviewsSection listingId={listing.id} currentUser={currentUser} addToast={addToast} onReviewAdded={onReviewAdded} />

        {/* Mehr vom Anbieter */}
        {(() => {
          const ownerOthers = (allListings || []).filter((l) => l.userId === listing.userId && l.id !== listing.id).slice(0, 3);
          if (!ownerOthers.length) return null;
          return (
            <div style={{ marginTop: "3rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
                <h2 style={{ color: C.forest, fontSize: "1.5rem", margin: 0, letterSpacing: "-0.02em" }}>Mehr von {listing.ownerName}</h2>
                <span style={{ fontSize: "0.82rem", color: C.muted }}>{ownerOthers.length} weitere{ownerOthers.length !== 1 ? "" : "s"} Inserat{ownerOthers.length !== 1 ? "e" : ""}</span>
              </div>
                ...
                {ownerOthers.map((item) => (
                  <div key={item.id} className="hover-card" style={{ background: "white", borderRadius: 20, overflow: "hidden", border: `1px solid ${C.line}`, boxShadow: C.shadow, cursor: "pointer" }} onClick={() => { window.scrollTo(0, 0); onSelectListing(item); }}>
                    <div style={{ height: 150, background: C.sageLight, position: "relative" }}>
                      ...
                      {item.isAvailable === false && <div style={{ position: "absolute", top: 8, left: 8, background: C.terra, color: "white", padding: "0.2rem 0.55rem", borderRadius: 999, fontSize: "0.68rem", fontWeight: 800 }}>Vergeben</div>}
                    </div>
                    <div style={{ padding: "1rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: "0.4rem", marginBottom: "0.3rem" }}>
                        <h3 style={{ color: C.forest, margin: 0, fontSize: "0.95rem", lineHeight: 1.2 }}>{item.title}</h3>
