...
export default function ListingsPage({ listings, loading, goTo, onSelectListing, currentUser, favorites, toggleFavorite, initCategory, initSearch }) {
  ...
  const locationOptions = ["Alle", ...new Set(listings.map((item) => item.location).filter(Boolean))].slice(0, 8);
  ...
  const filteredListings = listings
    .filter((item) => {
      ...
      const matchesLocation = locationFilter === "Alle" || item.location === locationFilter;
      const matchesUser = mode === "all" || (mode === "mine" && currentUser && item.userId === currentUser.id) || (mode === "favorites" && favorites.includes(String(item.id)));
      return matchesSearch && matchesCategory && matchesLocation && matchesUser;
    })
    .filter((item) => {
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        {locationOptions.length > 2 && (
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.5rem", alignItems: "center" }}>
            <span style={{ fontSize: "0.78rem", color: C.muted, fontWeight: 600, whiteSpace: "nowrap" }}>📍 Ort:</span>
            {locationOptions.map((loc) => (
              <button key={loc} onClick={() => setLocationFilter(loc)} style={{ padding: "0.35rem 0.75rem", borderRadius: 999, border: `1px solid ${C.line}`, background: locationFilter === loc ? C.terra : "white", color: locationFilter === loc ? "white" : C.muted, fontWeight: 600, cursor: "pointer", fontSize: "0.8rem", whiteSpace: "nowrap", transition: "all 0.2s ease" }}>
                {loc}
              </button>
            ))}
          </div>
        ...
        {/* Price filter */}
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.5rem", alignItems: "center" }}>
          <span style={{ fontSize: "0.78rem", color: C.muted, fontWeight: 600, whiteSpace: "nowrap" }}>💶 Preis:</span>
          {["Alle", "bis 5 €", "bis 10 €", "bis 20 €", "über 20 €"].map((opt) => (
            <button key={opt} onClick={() => setPriceFilter(opt)} style={{ padding: "0.35rem 0.75rem", borderRadius: 999, border: `1px solid ${C.line}`, background: priceFilter === opt ? C.gold : "white", color: priceFilter === opt ? "white" : C.muted, fontWeight: 600, cursor: "pointer", fontSize: "0.8rem", whiteSpace: "nowrap", transition: "all 0.2s ease" }}>
              {opt}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "1.2rem" }}>
          {loading ? [1,2,3,4,5,6].map((n) => <SkeletonCard key={n} />) : visibleListings.map((item, i) => (
            <ScrollReveal key={item.id} delay={Math.min(i * 60, 300)}>
              <div className="hover-card" style={{ background: "white", borderRadius: 20, overflow: "hidden", border: `1px solid ${item.isAvailable === false ? C.terra : C.line}`, boxShadow: C.shadow, cursor: "pointer", height: "100%", opacity: item.isAvailable === false ? 0.72 : 1 }} onClick={() => onSelectListing(item)}>
                <div style={{ height: 190, background: C.sageLight, position: "relative", overflow: "hidden" }}>
                  ...
                  {item.isAvailable === false && (
                    <div style={{ position: "absolute", top: 10, left: 10, background: C.terra, color: "white", padding: "0.3rem 0.7rem", borderRadius: 999, fontSize: "0.72rem", fontWeight: 800, letterSpacing: "0.04em" }}>Vergeben</div>
                  )}
                  {item.userId === "demo" && (
                    <div style={{ position: "absolute", top: 10, left: 10, background: "rgba(0,0,0,0.45)", color: "white", padding: "0.2rem 0.55rem", borderRadius: 999, fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.05em" }}>Beispiel</div>
                  )}
                  <button onClick={(e) => { e.stopPropagation(); toggleFavorite(item.id); }} style={{ position: "absolute", top: 10, right: 10, width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.9)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem", zIndex: 1, color: favorites.includes(String(item.id)) ? C.terra : C.muted }}>
        ...
        {!loading && hasMore && (
          <div style={{ gridColumn: "1 / -1", textAlign: "center", paddingTop: "1rem" }}>
            <button onClick={() => setVisibleCount((c) => c + 12)} style={{ padding: "0.85rem 2.5rem", borderRadius: 999, border: `1px solid ${C.line}`, background: "white", color: C.forest, fontWeight: 700, cursor: "pointer", fontSize: "0.95rem", boxShadow: "0 4px 16px rgba(28,58,46,0.07)", transition: "all 0.2s ease" }}>
        ...
        {!loading && filteredListings.length === 0 && (() => {
          ...
          return <EmptyState title="Keine Inserate gefunden" text="Versuche einen anderen Suchbegriff, eine andere Kategorie oder entferne den aktiven Filter." buttonLabel="Filter zurücksetzen" onClick={() => { setSearch(""); setCategoryFilter("Alle"); setLocationFilter("Alle"); setPriceFilter("Alle"); setMode("all"); }} />;
        })()}
      </div>
    </div>
  );
