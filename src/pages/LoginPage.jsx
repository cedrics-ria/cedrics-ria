    ...
    <div style={{ minHeight: "100vh", background: C.cream, display: "flex" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem 1.5rem" }}>
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          {error && <div id="login-error" role="alert" aria-live="assertive" style={{ padding: "0.9rem 1rem", borderRadius: 14, background: "rgba(196,113,74,0.12)", color: C.terra, fontWeight: 600, marginBottom: "1rem", fontSize: "0.88rem", lineHeight: 1.5 }}>{error}</div>}

          <form onSubmit={handleSubmit} aria-label={mode === "login" ? "Anmeldeformular" : "Registrierungsformular"} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {mode === "register" && (
              <div>
                <label htmlFor="login-name" style={{ display: "block", marginBottom: "0.4rem", fontSize: "0.85rem", fontWeight: 700, color: C.forest }}>Name <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
                <input id="login-name" value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder="Dein Vorname" autoComplete="name" aria-required="true" aria-invalid={!!error && !name} aria-describedby={error ? "login-error" : undefined} onFocus={applyInputFocus} onBlur={resetInputFocus} style={inputBaseStyle} />
              </div>
            )}
            <div>
              <label htmlFor="login-email" style={{ display: "block", marginBottom: "0.4rem", fontSize: "0.85rem", fontWeight: 700, color: C.forest }}>E-Mail <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
              <input id="login-email" value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="du@uni-paderborn.de" autoComplete="email" aria-required="true" aria-invalid={!!error} aria-describedby={error ? "login-error" : undefined} onFocus={applyInputFocus} onBlur={resetInputFocus} style={inputBaseStyle} />
            </div>
            <div>
              <label htmlFor="login-password" style={{ display: "block", marginBottom: "0.4rem", fontSize: "0.85rem", fontWeight: 700, color: C.forest }}>Passwort <span aria-hidden="true" style={{ color: C.terra }}>*</span></label>
              <input id="login-password" value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Mindestens 6 Zeichen" autoComplete={mode === "login" ? "current-password" : "new-password"} aria-required="true" aria-invalid={!!error} aria-describedby={error ? "login-error" : undefined} onFocus={applyInputFocus} onBlur={resetInputFocus} style={inputBaseStyle} />
            </div>
            ...
            {mode === "login" && !forgotMode && (
              <button type="button" onClick={() => setForgotMode(true)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: "0.85rem", textDecoration: "underline", padding: "0.25rem 0" }}>
                Passwort vergessen?
              </button>
            )}
            {forgotMode && !forgotSent && (
              <div style={{ borderTop: `1px solid ${C.line}`, paddingTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                <p style={{ margin: 0, fontSize: "0.88rem", color: C.muted, lineHeight: 1.5 }}>Gib deine E-Mail-Adresse ein — wir schicken dir einen Reset-Link.</p>
                <input type="email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} placeholder="deine@email.de" onFocus={applyInputFocus} onBlur={resetInputFocus} style={inputBaseStyle} />
                <div style={{ display: "flex", gap: "0.6rem" }}>
                  <button type="button" onClick={() => setForgotMode(false)} style={{ flex: 1, padding: "0.85rem", borderRadius: 12, border: `1px solid ${C.line}`, background: "white", color: C.forest, fontWeight: 700, cursor: "pointer" }}>Abbrechen</button>
                  <button type="button" onClick={async () => {
                    if (!forgotEmail) return;
                    await supabase.auth.resetPasswordForEmail(forgotEmail, { redirectTo: window.location.origin });
                    setForgotSent(true);
                  }} style={{ flex: 2, ...primaryButtonStyle, borderRadius: 12, padding: "0.85rem" }}>Link senden</button>
                </div>
              </div>
            )}
            {forgotSent && (
              <div style={{ background: C.sageLight, borderRadius: 12, padding: "0.85rem 1rem", textAlign: "center", fontSize: "0.88rem", color: C.forest, fontWeight: 600 }}>
